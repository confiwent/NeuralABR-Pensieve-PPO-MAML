...
def evaluation(model, log_path_ini, net_env, all_file_name, detail_log = True):
    ...
    for video_count in tqdm(range(len(all_file_name))):
        while True:
            with torch.no_grad():
                prob, _ = model(state.unsqueeze(0).type(torch.FloatTensor))
...
def valid(shared_model, epoch, log_file, summary_dir, name_str):
                
    os.system('rm -r ' + summary_dir + '/test_results/')
    os.system('mkdir ' + summary_dir + '/test_results/')

    model = ActorCritic(A_DIM).type(torch.FloatTensor)
    model.eval()
    model.load_state_dict(shared_model.state_dict())
    log_path_ini = summary_dir + '/test_results/log_valid_pppo'
    ...
    rewards = []
    test_log_files = os.listdir(summary_dir + '/test_results/')
    for test_log_file in test_log_files:
        reward = []
        with open(summary_dir + '/test_results/' + test_log_file, 'rb') as f:
            for line in f:
                parse = line.split()
                try:
    ...
    # add_str = 'ppo'
    model_save_path = summary_dir + "/%s_%s_%d.model" %(str('abr'), name_str, int(epoch))
