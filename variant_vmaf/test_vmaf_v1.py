...
from utils.helper import get_opt_space_a2br
        ...
        while True:
            ...
            if cur_index == total_chunk_num:
                ...
                action_mask, future_bandwidth = \
                    get_opt_space_a2br(
                                state, last_quality, cur_index, bitrate_versions, 
                                quality_penalty, rebuffer_penalty, smooth_penalty_p, 
...
def valid(args, env, actor_net, epoch, log_file, save_folder, add_str = 'a2c'):
    ...
    s_info, s_len, c_len, total_chunk_num, bitrate_versions, \
        quality_penalty, rebuffer_penalty, smooth_penalty_p, smooth_penalty_n = env.get_env_info()
    a_dim = len(bitrate_versions)

    actor_net.eval()

    evaluation(actor_net, log_path_ini, env, \
                    s_info, s_len, c_len, total_chunk_num, bitrate_versions, \
                        quality_penalty, rebuffer_penalty, smooth_penalty_p, \
                            smooth_penalty_n, a_dim, args)
...
def test(args, test_model, env, log_file, summary_dir):
    ...
    evaluation(model_actor, log_file, env, \
        s_info, s_len, c_len, total_chunk_num, bitrate_versions, \
            quality_penalty, rebuffer_penalty, smooth_penalty_p, \
                smooth_penalty_n, a_dim, args)
