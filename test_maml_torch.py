...
def evaluation(
    model, log_path_ini, net_env, all_file_name, detail_log=True, q_lin=False
):
        ...
        while True:
            ...
            if detail_log == False:
                ...
                if q_lin:
                    ...
                    # -- log scale reward --
                    log_bit_rate = np.log(
                        VIDEO_BIT_RATE[bit_rate] / float(VIDEO_BIT_RATE[0])
                    )
                    log_last_bit_rate = np.log(
                        VIDEO_BIT_RATE[last_bit_rate] / float(VIDEO_BIT_RATE[0])
                    )
                    reward = (
                        log_bit_rate
...
def valid(args, shared_model, epoch, log_file, test_log_path):
    ...
    evaluation(model, log_path_ini, env, all_file_names, False, q_lin=args.lin)
...
def test(test_model, test_traces, log_file):
    ...
    log_path_ini = log_file + "log_test_maml"
